...
for epoch in range(NUM_EPOCHS):
    print(f'Epoch: {epoch}')
    t_losses, v_losses = [], []
    val_size = int(np.ceil(data.shape[0]/5))
    for i in range(5):
        start = i*val_size
        stop = (i+1)*val_size if (i+1)*val_size <= data.shape[0] else data.shape[0]
        ctdata = np.delete(data, np.arange(start,stop), axis=0)
        cvdata = data[start:stop]
...
torch.save(model, 'model_3layers_crossvalidation.mdl')